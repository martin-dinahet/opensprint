...
describe("useCreateTaskForm", () => {
  ...
  it("creates a task and resets local form state", async () => {
    ...
    const { result } = renderHook(() => useCreateTaskForm({ columnId: "column-1", onOpenChange }));
  ...
  it("stores validation errors without submitting", async () => {
    const { result } = renderHook(() => useCreateTaskForm({ columnId: "column-1", onOpenChange: vi.fn() }));
  ...
  it("shows a global error when no column is selected", async () => {
    ...
    await waitFor(() => expect(result.current.globalError).toBe("Choose a column before creating a task."));
    expect(createTaskMock.mutateAsync).not.toHaveBeenCalled();
  });
