import type { BoardOutput, CreateBoardInput, UpdateBoardInput } from "@/entities/board";
...
export const boardKeys = {
  ...
  detail: (projectId: string, boardId: string) => [...boardKeys.details(), projectId, boardId] as const,
} as const;
...
export const boardApi = {
  ...
  update: async (projectId: string, boardId: string, data: UpdateBoardInput) => {
    return requestApiResult<BoardOutput>(
      () => api.projects[":id"].boards[":boardId"].$patch({ param: { id: projectId, boardId }, json: data }),
