...
describe("task API", () => {
  ...
  it("lists tasks and normalizes missing arrays", async () => {
    apiMock.boards[":boardId"].tasks.$get.mockResolvedValue({});

    const result = await taskApi.list("board-1");

    expect(result.unwrap()).toEqual({ tasks: [] });
    expect(apiMock.boards[":boardId"].tasks.$get).toHaveBeenCalledWith({ param: { boardId: "board-1" } });
  ...
  it("passes params and bodies to task mutations", async () => {
    apiMock.boards[":boardId"].tasks.$post.mockResolvedValue({ id: "task-new" });
    apiMock.boards[":boardId"].tasks[":taskId"].$patch.mockResolvedValue({ id: "task-1" });
    apiMock.boards[":boardId"].tasks[":taskId"].$delete.mockResolvedValue({ success: true });
    apiMock.tasks[":taskId"].assign.$patch.mockResolvedValue({ id: "task-1", assigneeId: null });
    apiMock.tasks[":taskId"].move.$patch.mockResolvedValue({ id: "task-1", boardId: "board-2", position: 1 });
    ...
    await taskApi.move("task-1", { boardId: "board-2", position: 1 });
    await taskApi.reorder("task-1", { position: 2 });

    expect(apiMock.boards[":boardId"].tasks.$post).toHaveBeenCalledWith({
      param: { boardId: "board-1" },
      json: { title: "Ship it", priority: "high" },
    });
    expect(apiMock.boards[":boardId"].tasks[":taskId"].$patch).toHaveBeenCalledWith({
      param: { boardId: "board-1", taskId: "task-1" },
      json: { title: "Shipped" },
    });
    expect(apiMock.boards[":boardId"].tasks[":taskId"].$delete).toHaveBeenCalledWith({
      param: { boardId: "board-1", taskId: "task-1" },
    });
    ...
    expect(apiMock.tasks[":taskId"].move.$patch).toHaveBeenCalledWith({
      param: { taskId: "task-1" },
      json: { boardId: "board-2", position: 1 },
    });
