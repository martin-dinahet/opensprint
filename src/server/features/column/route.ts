...
const CreateColumnSchema = CreateColumnInput;
const UpdateColumnSchema = UpdateColumnInput;
const ReorderColumnsSchema = ReorderColumnsInput;
...
export const columnRoute = new Hono<ServerVariables>() //
  ...
  .post("/:boardId/columns", guard(), validate("json", CreateColumnSchema), async (c) => {
    const boardId = c.req.param("boardId");
    const currentUser = c.get("user");
  ...
  .get("/:boardId/columns/:columnId", guard(), async (c) => {
    const boardId = c.req.param("boardId");
    const columnId = c.req.param("columnId");
    const currentUser = c.get("user");

    const result = await getColumn(currentUser.id, boardId, columnId);
  ...
  .patch("/:boardId/columns/reorder", guard(), validate("json", ReorderColumnsSchema), async (c) => {
    const boardId = c.req.param("boardId");
    const currentUser = c.get("user");
    const body = c.req.valid("json");
  ...
  .patch("/:boardId/columns/:columnId", guard(), validate("json", UpdateColumnSchema), async (c) => {
    const boardId = c.req.param("boardId");
    const columnId = c.req.param("columnId");
    const currentUser = c.get("user");
    const body = c.req.valid("json");
